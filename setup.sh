...
install_required_tools() {
  ...
  # 3. Install pipx-managed tools (Semgrep, Autotools Language Server)
  echo -e "\033[1;32m\n[ Checking Isolated Dependencies (pipx) ]\033[0m"

  # Ensure pipx path is registered (shared for all pipx tools)
  ...
  if ! command -v autotools-language-server >/dev/null 2>&1; then
    echo "Installing autotools-language-server via pipx..."
    pipx install autotools-language-server
  else
    echo "autotools-language-server is already installed via pipx."
  fi
}
