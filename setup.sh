...
menu_select_actions() {
  ...
  while true; do
    ...
    for i in "${!_indices[@]}"; do
      ...
      # Visual styling for the current cursor position
      if [[ $i -eq $cursor ]]; then
        pointer="${TITLE_COLOR}> ${NC}"
        # We can add a subtle bold or color to the line itself
        echo -ne "${pointer}"
      else
        echo -ne "  "
      fi
      ...
      if [[ $i -eq $cursor ]]; then
        echo -e "${bracket} ${TITLE_COLOR}${name}${NC}$(tput el)"
      else
        echo -e "${bracket} ${name}$(tput el)"
      fi
    done

    # --- Legend ---
    echo -e "${TITLE_COLOR}────────────────────────────────────────────────────${NC}"
    # Using printf for exact alignment
    printf " %b↑/↓/n/p%b Navigate  %bSPACE%b Toggle  %bENTER%b Run  %bA%b Abort\n" \
        "${KEY_COLOR}" "${NC}" \
        "${KEY_COLOR}" "${NC}" \
        "${CHECK_COLOR}" "${NC}" \
        "${ERR_COLOR}" "${NC}"
    echo -e "${TITLE_COLOR}────────────────────────────────────────────────────${NC}"
    tput ed # Clear everything below the legend to prevent ghosting

    # Use -d '' to prevent 'read' from stopping at a newline character
    # and IFS= to preserve the literal character
    IFS= read -rsn1 -d '' key

    # Handle Escape Sequences (Arrows)
    if [[ $key == $'\e' ]]; then
      read -rsn2 -t 0.05 next_chars
      case "$next_chars" in
        "[A") key="UP" ;;
        "[B") key="DOWN" ;;
      esac
    ...
    case "$key" in
      ...
      "a"|"A")
        tput cnorm; clear; log_error "Setup aborted."; exit 1
        ;;
    esac
  done
...
install_system_packages() {
  ...
  if [ -f /etc/debian_version ]; then
    cmd="sudo DEBIAN_FRONTEND=noninteractive apt update && sudo apt install -y ${packages[*]}"
    local os_label="Debian/Ubuntu"
  elif [ -f /etc/fedora-release ] || command -v dnf >/dev/null 2>&1; then
    cmd="sudo dnf install -y ${packages[*]}"
    local os_label="Fedora/RHEL"
  else
    log_error "OS not supported for auto-installation. Please install $friendly_name manually."
    return 1
  fi

  run_task "System: Installing $friendly_name ($os_label)" "$cmd"
}
