...
(defun cfg-core/--layers-includes-manifest ()
  "Return the absolute path of layers/includes.org."
  (expand-file-name "includes.org" (cfg-core/--layers-dir)))
...
(defun cfg-core/--layers-index-content (files layers-dir)
...
(defun cfg-core/--layers-includes-content (files layers-dir)
  "Return full layers/includes.org text for sorted FILES under LAYERS-DIR.
Deliberately headline-free: GitHub renders headlines but silently drops
unresolved #+INCLUDE keywords, so this file is invisible on GitHub while
`org-export-expand-include-keyword' still resolves it at tangle time."
  (let ((include-lines (cfg-core/--layer-include-lines files layers-dir)))
    (concat "# Auto-generated by cfg-core/discover-layers -- do not edit by hand\n"
            "\n"
...
(defun cfg-core/discover-layers ()
  "Scan layers/**/README.org and rewrite layers/README.org + layers/includes.org.
Orders by category PRIORITY, then #+LAYER_PRIORITY, then path.
Reverts visiting manifest buffers after updating."
  (let* ((layers-dir (cfg-core/--layers-dir))
         (manifest (cfg-core/--layers-manifest))
         (includes-manifest (cfg-core/--layers-includes-manifest))
         (files (cfg-core/--sort-layer-files
                 (cfg-core/--layer-readme-files layers-dir)
                 layers-dir)))
    (cfg-core/--write-layers-manifest
     manifest
     (cfg-core/--layers-index-content files layers-dir))
    (cfg-core/--write-layers-manifest
     includes-manifest
     (cfg-core/--layers-includes-content files layers-dir))))
