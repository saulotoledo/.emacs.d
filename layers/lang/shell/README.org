...
- *Static Analysis*: Integrated linting via =shellcheck= to catch syntax errors,
  portability issues, and common logic bugs early.
- *Interactive Shell Completion*: Enhanced command-line completion in
  ~shell-mode~ buffers using ~bash-completion~.

* Emacs Configuration
#+begin_src emacs-lisp
(cfg-editor/register-treesit-auto-language 'bash)
#+end_src
...
(cfg-keys/mode-leader-def (sh-mode shell-mode)
  "m"   '(:ignore t :which-key "major-mode")
  "m c" '(bash-completion-mode :which-key "Toggle Bash Completion"))
#+end_src
